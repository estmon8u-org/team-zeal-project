...
- **Run Streamlit UI Locally:**
    ...
    # Ensure API_URL in streamlit_app.py is correct for your local/deployed API
    # or set PREDICTION_API_URL environment variable
    # export PREDICTION_API_URL="http://localhost:8008/predict/" # If API running locally
...
- **Docker Images:** Stored in GCP Artifact Registry (e.g., `YOUR_REGION-docker.pkg.dev/for-copying-332923/team-zeal-project/...`).
...
- **Deployment:** See `PHASE3.md` or section 11.2 below for `gcloud run deploy` commands.
- **Live API Endpoint (Example):** `https://team-zeal-api-run-1004281831193.us-west2.run.app/predict/` *(Replace with your actual deployed URL)*
- **Local API Run:** `make api_run` (ensure `MODEL_GCS_PATH` is set).
...
1. **Build and Push API Docker Image:**
    ...
    ```bash
    make api_docker_push_gcp API_IMAGE_TAG=your_api_tag
    ```
...
2. **Deploy to Cloud Run:**
    ...
    export GCP_PROJECT_ID="for-copying-332923" # Your GCP Project ID
    ...
    export MODEL_GCS_PATH_FOR_SERVICE="gs://team-zeal-models/main/3f941fb_model.pth" # Your model in GCS
...
3. **Test:**
    ...
    ```bash
    curl -X POST -F "file=@/path/to/image.jpg" YOUR_DEPLOYED_CLOUD_RUN_URL/predict/
    ```
...
# e.g., export PREDICTION_API_URL="https://team-zeal-api-run-1004281831193.us-west2.run.app/predict/"
streamlit run streamlit_app.py
cd ..
```
